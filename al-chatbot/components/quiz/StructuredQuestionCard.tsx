...
interface SubState {
    ...
    answer: string;
    submittedAnswer: string;          // frozen copy shown after evaluation
    imageFile: File | null;
...
export default function StructuredQuestionCard({question, subject, config, onScored, onNext, onPrev}: Props) {
    ...
    async function handleSubmit(idx: number) {
        ...
        // Freeze the text answer before clearing state for the new evaluation
        const frozenAnswer = state.inputMode === "text" ? state.answer : "";
    ...
    return (
        <div style={{display: "flex", flexDirection: "column", gap: "16px", paddingRight: "10px"}}>
            ...
            <div style={{...card, borderLeft: "4px solid", borderLeftColor: config.activeDot as string}}>
                <div style={{padding: "20px 24px"}}>
                    <div style={{display: "flex", alignItems: "center", gap: "8px", marginBottom: "12px"}}>
                        <span className={`${config.badgeBg} ${config.textColor}`} style={{
                        ...
                        <div style={{display: "flex", flexWrap: "wrap", gap: "6px"}}>
                            {question.topics.map((t) => (
                                <span key={t} style={{
                                    fontSize: "11px", color: "#9ca3af", padding: "2px 8px",
            ...
            {question.questions.map((sub, idx) => {
                ...
                const markingPoints = (sub.marking_points ?? []) as Array<{ point: string; marks: number }>;
                ...
                return (
                    <div key={sub.id} style={{...card}}>
                        <div style={{padding: "24px"}}>
                            ...
                            <div style={{
                                ...
                                <div style={{display: "flex", alignItems: "flex-start", gap: "10px"}}>
                                    ...
                                    <MathText text={sub.question} style={{
                                        fontSize: "16px", fontWeight: 600, color: "#111827", lineHeight: 1.6
                                    }}/>
                                </div>
                                <span style={{
                                    fontSize: "13px", color: "#6b7280", background: "#f9fafb",
                                    padding: "3px 12px", borderRadius: "9999px", border: "1px solid #e5e7eb",
                            ...
                            {sub.diagram_reference && (
                                <div style={{marginTop: "4px", marginBottom: "16px"}}>
                                    ...
                                    <img
                                        ...
                                        alt={`Diagram for sub-question ${roman}`}
                                        style={{
                                            maxWidth: "100%", borderRadius: "8px",
                                            border: "1px solid #e5e7eb", background: "#f9fafb", display: "block"
                                        }}
                            ...
                            {/* Input mode toggle — only before result */}
                            {!state.result && (
                            ...
                            {/* Image input — only before result */}
                                ...
                                <div>
                                    ...
                                    {!state.imagePreview ? (
                                        <button
                                            ...
                                            style={{
                                                width: "100%", padding: "24px 16px", borderRadius: "10px",
                                        ...
                                        >
                                            ...
                                            <p style={{fontSize: "11px", color: "#9ca3af"}}>
                                                Handwritten answers, calculations, diagrams
                                            </p>
                                        </button>
                                    ) : (
                                        <div style={{
                            ...
                            {/* Submit button — only before result */}
                            {!state.result && (
                                <div style={{marginTop: "12px"}}>
                                    ...
                                    >
                                        {state.loading
                                            ...
                                            : state.inputMode === "image" ? "Evaluate Image →" : "Submit Answer →"
                                        }
                                    </Button>
                                </div>
                            )}
                            ...
                            {state.streamBuffer && !state.result && (
                                <div style={{
                                    ...
                                    <pre style={{
                                        ...
                                        fontFamily: "monospace", lineHeight: 1.5, maxHeight: "80px", overflow: "hidden"
                            ...
                            {state.result && (
                                <div style={{display: "flex", flexDirection: "column", gap: "12px", marginTop: "4px"}}>
                                        ...
                                        <div style={{
                                            ...
                                            <div style={{display: "flex", alignItems: "center", gap: "10px"}}>
                                                <span style={{fontSize: "26px", fontWeight: 700, color: "#111827"}}>
                                                    {state.result.marksAwarded}/{state.result.totalMarks}
                                                </span>
                                                <span style={{fontSize: "13px", color: "#6b7280"}}>marks</span>
                                                <span style={{
                                                    fontSize: "12px", fontWeight: 500, padding: "2px 10px",
                                                    borderRadius: "9999px", background: sc.badge, color: sc.badgeText
                                                }}>{scoreLabel(pct)}</span>
                                            </div>
                                            <span style={{fontSize: "20px", fontWeight: 700, color: sc.text}}>{pct}%</span>
                                        </div>
                                        <div style={{height: "6px", borderRadius: "9999px", background: "#e5e7eb", overflow: "hidden"}}>
                                            <div style={{
                                    ...
                                    {/* Your submitted text answer */}
                                    {state.inputMode === "text" && state.submittedAnswer && (
                                        <div style={{
                                    ...
                                    {/* Your submitted image answer */}
                                    {state.inputMode === "image" && state.imagePreview && (
                                        <div style={{
                                            padding: "14px 16px", background: "#f9fafb",
                                            borderRadius: "10px", border: "1px solid #e5e7eb"
                                        }}>
                                            ...
                                            <img src={state.imagePreview} alt="Submitted answer" style={{
                                    ...
                                    <div style={{borderRadius: "10px", border: "1px solid #e5e7eb", overflow: "hidden"}}>
                                        <button
                                            onClick={() => updateSub(idx, {correctAnswerOpen: !state.correctAnswerOpen})}
                                            style={{
                                        ...
                                        >
                                            ...
                                            <ChevronDown style={{
                                                ...
                                                transform: state.correctAnswerOpen ? "rotate(180deg)" : "rotate(0deg)"
                                            }}/>
                                        </button>

                                        {state.correctAnswerOpen && (
